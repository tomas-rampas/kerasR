...
#' 
#' Returns TRUE if the python `keras` library is installed. If the function returns FALSE, but you believe `keras` is installed, then see [reticulate::use_python] to configure the python environment, and then try running [keras_init] to establish the connection to `keras`.
#' 
#' @export
#' @return Logical
#' @seealso [keras_init]
keras_available <- function(){
  ...
  else if(!py_numpy_available()) 
    msg <- collapse(msg, "numpy  not available")
  else if(!py_module_available("keras")) 
...
#' 
#' @importFrom  reticulate import import_builtins py_available py_module_available py_numpy_available
#' @export
#' @seealso [keras_available]
keras_init <- function(){
  if(keras_available()){
    
    modules$builtin <-  import_builtins()
    ...
    modules$keras.layers.advanced_activations <- import("keras.layers.advanced_activations")
    ...
    modules$keras.preprocessing.sequence <- import("keras.preprocessing.sequence")
