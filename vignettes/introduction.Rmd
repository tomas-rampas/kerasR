...
of general purpose layers. In this vignette we illustrate the basic
...
method name. Layers are added by calling the method, written as
a function in $, `add`. This function takes as an input another
`python.builtin.object`, generally constructed as the output of
another **kerasR** function. For example, to add a dense layer to
our model we do the following:
...
squared. Next in the model, we add an activation defined by a rectified
linear unit to the model:
...
as well. By default it is encoding in a one-column matrix with each row
